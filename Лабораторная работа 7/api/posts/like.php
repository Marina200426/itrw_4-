...
/**
 * Endpoint для добавления лайка к статье
...
if (!isset($input['post_uuid']) || !isset($input['user_uuid'])) {
    ob_clean();
    http_response_code(400);
...
try {
    ...
    // Проверяем, не поставил ли уже пользователь лайк этой статье
    if ($likesRepository->exists($postUuid, $userUuid)) {
        ob_clean();
        http_response_code(409); // Conflict
        echo json_encode([
            'error' => 'User has already liked this post',
            'message' => 'Пользователь уже поставил лайк этой статье'
        ]);
        ob_end_flush();
        exit;
    }
